...
class Connection():
    ...
    async def run(self, cmds_q, results_q):
        async with asyncssh.connect(self.target, port=self.port, username=self.username, password=self.password) as connection:
            async with connection.create_process("bash") as cpr:
                while True:
                    logging.warning("=== :: Check the queue for pending commands.")
                    do_it = await cmds_q.get()
                    args = [ "ls", "/" "\n"]
                    logging.warning("--- Write {args} into stdin")
                    cpr.stdin.write('ls\n')
